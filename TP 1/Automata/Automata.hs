...
probarPosYNeg :: [Estado] -> [[Char]] -> [[Char]] -> [((Estado, Char), Estado)] -> Bool
probarPosYNeg estadosFinales poss negs fun = (correPositivos estadosFinales poss fun) && (noCorreNegativos estadosFinales negs fun)

correPositivos :: [Estado] -> [[Char]] -> [((Estado, Char), Estado)] -> Bool
correPositivos estadosFinales [pos] fun = correCadena estadosFinales (Estado 0) pos fun
correPositivos estadosFinales (p:poss) fun = (correCadena estadosFinales (Estado 0) p fun) && (correPositivos estadosFinales poss fun)

noCorreNegativos :: [Estado] -> [[Char]] -> [((Estado, Char), Estado)] -> Bool
noCorreNegativos estadosFinales [neg] fun = not (correCadena estadosFinales (Estado 0) neg fun)
noCorreNegativos estadosFinales (n:negs) fun = (not (correCadena estadosFinales (Estado 0) n fun)) && (noCorreNegativos estadosFinales negs fun)

correCadena :: [Estado] -> Estado -> [Char] -> [((Estado, Char), Estado)] -> Bool
correCadena estadosFinales estadoActual [] fun = pertenece estadoActual estadosFinales
correCadena estadosFinales estadoActual (p:pos) fun = correCadena estadosFinales (funFromTuples fun (estadoActual, p)) pos fun
...
obtenerEstadosYFuncion (p:posEstadosFinales) alf poss negs (f:fun) | (probarPosYNeg p poss negs f) = (p, f)
													 	 		 | otherwise = notEmpty (obtenerEstadosYFuncion (p:posEstadosFinales) alf poss negs fun) (obtenerEstadosYFuncion posEstadosFinales alf poss negs (f:fun))
